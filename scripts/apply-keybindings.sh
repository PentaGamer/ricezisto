...
# 3. Definir Kitty como Terminal Padrão do Desktop GNOME
gsettings set org.gnome.desktop.default-applications.terminal exec 'kitty'
gsettings set org.gnome.desktop.default-applications.terminal exec-arg '-e'
gsettings set org.gnome.settings-daemon.plugins.media-keys terminal "@as []"

# 4. Atalhos Customizados (Terminal, Launcher, Powermenu e Clipboard)
python3 -c "
import subprocess
...
bindings = [
    ...
    ('custom4', 'Terminal Kitty (Ctrl+Alt+T)', 'kitty', '<Primary><Alt>t')
]
...
echo "  -> Super + Return / Ctrl+Alt+T : Abrir Terminal Kitty (Catppuccin)"
